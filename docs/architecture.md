...
- **`Controllers/`** — MVC controllers. Added from Phase 2 onwards. Phase 3 adds `MarinasController` (`placeowner/marinas`), `SpotsController` (`placeowner/marinas/{marinaId}/spots`), `SpotSeasonalRulesController` (`placeowner/marinas/{marinaId}/spots/{spotId}/seasonal-rules`), and `BrowseController` (`browse/marina/{id}/layout-data`, `browse/marina/{id}/spot-statuses` — the latter returns `[{ id, status }]` where status is `Unavailable` (inactive) / `Booked` (active + a Pending/Confirmed booking overlapping today, via `IBookingRepository.GetOccupiedSpotIdsAsync`) / `Free`); also adds `GET /browse` (`BrowseController.Index`) — a temporary placeholder landing page (`Views/Browse/Index.cshtml`, reusing the workspace/empty-state components) added so the BoatOwner home redirect and "Back to browse" links resolve instead of 404'ing; the full Phase 7 Browse (public marina list + ES search + click-to-book) will replace it. Phase 4 adds `VesselsController` (`vessels`) — `[Authorize(Roles="BoatOwner")]`, actions: Index, Create GET/POST, Edit GET/POST, Delete POST. Phase 5 adds `BookingsController` (`bookings`) — `[Authorize(Roles="BoatOwner")]`, actions: MyBookings GET, Create GET/POST, PreviewPrice GET, Cancel POST, Dismiss POST; and `SpotBookingsController` (`placeowner/spot-bookings`) — `[Authorize(Roles="PlaceOwner")]`, actions: Incoming GET, Confirm POST, Reject POST, Cancel POST, Dismiss POST. Navigation: `_Layout.cshtml` shows an "Incoming Bookings" link (`/placeowner/spot-bookings`) for the PlaceOwner role. Phase 5b adds `BoatOwnerReviewsController` (`reviews`) — `[Authorize(Roles="BoatOwner")]`, actions: Create GET/POST; and `PlaceOwnerReviewsController` (`placeowner/reviews`) — `[Authorize(Roles="PlaceOwner")]`, actions: Create GET/POST. Both controllers share the view at `Views/Reviews/Create.cshtml`. Phase 6 adds `AdminController` (`admin`) — `[Authorize(Roles="Admin")]`. Read actions: Dashboard, Jobs (`/admin/jobs` — embeds the Hangfire dashboard in a same-origin iframe inside the site layout), Users, AllBookings, AllMarinas, MarinaSpots, MarinaAdmins, MarinaInvitations, MarinaLayout, Settings GET. Write actions: CreateMarina GET/POST, EditMarina GET/POST, ToggleMarinaActive POST, ToggleSpotActive POST, InviteAdmin GET/POST, RevokeAdmin POST, ReEnableAdmin POST, CancelBooking POST, Settings POST. (`RevokeAdmin` is a soft-revoke + `ReEnableAdmin` re-enable — see [`features/admin-and-invitations.md`](features/admin-and-invitations.md).) `HomeController.Index` (`home/`) serves the public landing page and doubles as the brand-logo target — it redirects signed-in users per role before rendering: Admin → `/admin/dashboard`, PlaceOwner → `/placeowner/marinas`, BoatOwner → `/browse`; guests and roleless users see the landing `View()`.
...
- **`wwwroot/js/`** — client-side JavaScript. `marina-editor.js` drives the PlaceOwner canvas layout editor (Konva stage, Add Spot modal, SavePositions POST, snap/overlap collision logic, sidebar delete modal, clear background modal, fullscreen toggle). `marina-viewer.js` is the read-only counterpart used by the Admin layout page (`Admin/MarinaLayout.cshtml`): it mirrors the editor's stage/background setup but with no Transformer/drag/save — it fetches `layout-data` + `spot-statuses` and draws placed spots colored by status (Free=green, Booked=amber, Unavailable=grey) with labels. `booking-dismiss.js` powers the AJAX dismiss on `Bookings/MyBookings.cshtml` and `SpotBookings/Incoming.cshtml` (fetch the dismiss endpoint with `Accept: application/json`, remove the card in place). Konva.js is loaded from CDN (`https://unpkg.com/konva@9/konva.min.js`) on the views that use the canvas — it is not bundled. See [`conventions.md`](conventions.md) § Canvas / Visual Layout for the full canvas ruleset.
...
9. Scoped repository and service registrations: `IAdminSettingsRepository`, `IInvitationRepository`, `IMarinaAdminRepository`, `IAuditLogger`, `IMarinaRepository`, `ISpotRepository`, `ISpotSeasonalRuleRepository`, `ISpotSeasonalRuleService`, `IFileStorageService`, `IVesselRepository`, `IBookingRepository`, `IBookingService`, `IReviewRepository`, `IReviewService`. (`IMarinaSearchService` and `IReviewSearchService` are registered in step 10.)
10. Elasticsearch config guard — reads `Elasticsearch:Uri` from configuration. If blank: registers `NullMarinaSearchService` as `IMarinaSearchService` (scoped) and `NullReviewSearchService` as `IReviewSearchService` (scoped). If set: registers `ElasticsearchClient` as singleton (with `DefaultIndex("marinas")`) then `ElasticsearchMarinaSearchService` as `IMarinaSearchService` (scoped) and `ElasticsearchReviewSearchService` as `IReviewSearchService` (scoped).
...
**Admin components (Phase 6).** Three new components live in `site.css`:
- `.console-grid` — staggered-animation grid of dashboard navigation cards.
- `.ledger` / `.ledger__row` / `.ledger__cell` — responsive data table used across all Admin list views. Stacks cells vertically on mobile (each cell shows its `data-label` as a `::before` pseudo-element header); at ≥720px it renders as a conventional multi-column table. `.ledger__row--inactive` dims rows for deactivated marinas/spots.
- `.canvas-legend` / `.canvas-legend__item` / `.canvas-legend__swatch` — status key shown below the read-only Konva canvas in `Admin/MarinaLayout.cshtml`. Swatches: `--free` (green fill), `--booked` (amber fill), and `--unavailable` (grey fill). The canvas is rendered by `marina-viewer.js` and is functional as of this session (previously the script did not exist and the canvas was inert).
- `.embed` / `.embed__frame` + the `.site-main--flush` `<main>` modifier — full-bleed iframe shell used by `Admin/Jobs.cshtml` to embed the Hangfire dashboard inside the site layout. The shared content column (`.site-header__inner` / `.site-main` / `.site-footer`) is capped at `max-width: 1320px`.
- `.workspace__back` — back-to-home link rendered at the top of section views' `workspace__head`. Used across all three roles: all Admin section views link to `/admin/dashboard` ("Back to dashboard") or `/admin/marinas` ("Back to marinas"); BoatOwner pages `Views/Bookings/MyBookings.cshtml` and `Views/Vessels/Index.cshtml` link to `/browse` ("Back to browse"); PlaceOwner pages `Views/Marinas/Layout.cshtml` and `Views/SpotBookings/Incoming.cshtml` link to `/placeowner/marinas` ("Back to my marinas").
...
**Validation.** Forms validate server-side only via ASP.NET Core ModelState. Each `<form>` carries the `novalidate` attribute to suppress the browser's default tooltip-style errors, so all errors render through `asp-validation-summary` (top-of-form `.auth-errors` block) and `asp-validation-for` (per-field `.field__error` span) on the next page load after a failed POST. No client-side JavaScript validation runs on text/ModelState forms. Two tech-lead-approved carve-outs exist:

1. **File input validation.** File uploads pre-validate size and MIME client-side (e.g. the marina background-image input in `marina-editor.js` checks `image/jpeg|png|webp`, allowed extensions, and the 5 MB limit before POSTing). Server-side validation in the controller stays as defense-in-depth and remains authoritative.
2. **Booking create form.** `Views/Bookings/Create.cshtml` runs vanilla JS that (a) fetches a live price preview from `GET /bookings/preview-price` when dates and vessel are selected, and (b) enforces a client-side "departure after arrival" check. Server-side validation (`BookingCreateViewModel` implements `IValidatableObject` — same error message) and the `BookingService.CreateAsync` domain guard remain authoritative. The form keeps `novalidate`; the JS layer is UX only.
