...
- `.canvas-legend` / `.canvas-legend__item` / `.canvas-legend__swatch` — status key shown below the read-only Konva canvas in `Admin/MarinaLayout.cshtml`. Swatches: `--free` (green fill) and `--unavailable` (grey fill).
